...
bool spi_verified_write_register(uint8_t reg, uint8_t num_bytes, uint8_t* writing_data){
    ...
    for (int i = 0; i < num_bytes; ++i) {
      ...
      if (read_data[i] != writing_data[i]) {
        Serial.println("Problem writing to SPI register");
        return true;
      } 
  }
  return false;
...
bool nRF24_tx_self_test() {
  ...
  uint8_t val = 0x00;
  spi_verified_write_register(W_REGISTER_MASK + EN_AA, 1, &val);        // disable auto acknowledgement  
  spi_verified_write_register(W_REGISTER_MASK + EN_RXADDR, 1, &val);    // disable RX data pipes
  spi_verified_write_register(W_REGISTER_MASK + SETUP_RETR, 1, &val);   // disable automatic re-transmit, ARC = 0000
  val = 0x0E;
  spi_verified_write_register(W_REGISTER_MASK + CONFIG, 1, &val);       // PWR_UP = 1 PRIMRX=0 (TX mode)
...
void RF_SEND() {
    ...
    Serial.print(nRF24_status, HEX); Serial.println(nRF24_status, HEX);
